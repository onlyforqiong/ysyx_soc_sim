...
#else
    // rt_kprintf("do this thread! name is %s\n",to_thread->name);
    
    //把read先丢出queen，然后再执行thread内容
    rt_hw_context_switch_to((rt_ubase_t)&to_thread->sp);
    // rt_kprintf("come the back\n");
#endif /* RT_USING_SMP */
...
void rt_schedule(void)
{
    ...
    // rt_kprintf("schedule rt_scheduler_lock_nest is %d\n",rt_scheduler_lock_nest);
    ...
    {
        rt_ubase_t highest_ready_priority;
        // rt_kprintf("rt_thread_ready_priority_group is %x\n",rt_thread_ready_priority_group);
        if (rt_thread_ready_priority_group != 0)
        {
            ...
            to_thread = _get_highest_priority_thread(&highest_ready_priority);
            // rt_kprintf("    hignest ready priority is %lx\n",highest_ready_priority);
            // rt_kprintf(" current priority  is %d and name is %s\n ",rt_current_thread->current_priority,rt_current_thread->name);
