...
class ysyx_22040886  extends Module with riscv_macros {
    ...
    val        io_master =  IO((new axi_ram_port))
...
// withClockAndReset(clk.asClock,(aresetn).asAsyncReset) {
    ...
    val _axi_cross_bar = Module(new axi_cross_bar_addr_switch(2,2,Array(0,0X02000000),Array(0,0X0200BFFF)))
    //length总共也就16，比较拉
  ...
  //  u_riscv_cpu.inst_cache_working_on := icache_first.cache_working_on    
    ...
    io_master <> _axi_cross_bar.io.s_port(0)
