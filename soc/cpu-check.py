...
stuNum = int(stuNum)
print("Your file "+"ysyx_22"+str(stuNum).zfill(4)+".v"+" will be check\n")
...
Error= 0
countTopModule = 0
for line in open("ysyx_22"+str(stuNum).zfill(4)+".v","r"): #设置文件对象并读取每一行文
    if "module " in line and "endmodule" not in line:
        if "module ysyx_22"+str(stuNum).zfill(4)+"_" in line:
            pass
        elif "module ysyx_22"+str(stuNum).zfill(4) in line and "module ysyx_22"+str(stuNum).zfill(4)+"_" not in line:
#             print(line)
...
newContent = []
for line in open("ysyx_22"+str(stuNum).zfill(4)+".v","r"): #设置文件对象并读取每一行文
    if "module ysyx_22"+str(stuNum).zfill(4) in line and "module ysyx_22"+str(stuNum).zfill(4)+"_" not in line:
        flag = 1
